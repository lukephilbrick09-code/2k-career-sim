...
async function createCareer(name) {
  const { data, error } = await sb.from('careers').insert({ name: name || 'My Player' }).select().single();
...
async function trainAttribute(attrId, vcCost, xpGain) {
  ...
  await sb.from('careers').update({ vc: newVc, overall: newOvr }).eq('id', currentCareerId);
...
async function signContract() {
  ...
  await sb.from('careers').update({ team, vc: cache.career.vc + bonus }).eq('id', currentCareerId);
...
async function addVc() {
  ...
  if (amt <= 0) return;
  await sb.from('careers').update({ vc: cache.career.vc + amt }).eq('id', currentCareerId);
