...
async function submitGame() {
  ...
  // Attributes are no longer auto-upgraded - total XP goes into the player's spendable bank instead.

  // Distribute to badges (own 6 categories) - badges still progress automatically from box score
  for (const [cat, names] of Object.entries(BADGE_DEFS)) {
  ...
  await sb.from('careers').update({ total_xp: newTotalXp, level: newLevel, banked_xp: newBankedXp }).eq('id', currentCareerId);

  toast(`Game logged: +${Math.round(totalXp)} XP added to your bank`);
  refreshCareer();
}
